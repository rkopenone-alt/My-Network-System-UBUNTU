    ...
    for root, _, files in os.walk(d):
        for file in files:
            if file.endswith('.kts'):
                ...
                # Revert Kotlin back to stable 1.9.24 to fix NoClassDefFoundError
                content = content.replace('version "2.1.20"', 'version "1.9.24"')
                
                if content != original_content:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    print(f"Fixed version in {file_path}")
